...
def have_a_word(k, dictionary=None):
    with open("/home/jovyan/Replication_study/measuring-execution/words_alpha.txt") as f:
        all_words = [i for i in f.read().splitlines()]
        if dictionary:
            available_words = list(set(all_words) - dictionary.keys())
            return random.sample(available_words, k)
        else:
...
class DictCreator:
    ...
    def create_dict(self) -> Dict[str, int]:
        ...
        if self.min_value > self.max_value:
            raise ValueError(f"Lower bound {self.min_value} cannot be greater than upper bound {self.max_value}")
        
        if self.min_value < 0 or self.max_value < 0:
            raise ValueError(f"Both values for dictionary creation have to be positive values")

        if self.seed is not None:
            random.seed(self.seed)

        result_dict = {}
        keys = have_a_word(self.num_pairs)

        # create random values from the range to one half of the keys, concatenate them with the opposite values and shuffle
        half_values = [random.randint(int(self.min_value), int(self.max_value)) for _ in range(self.num_pairs // 2)]
        values = half_values + [-value for value in half_values]
        random.shuffle(values)
        for index, word in enumerate(keys):
            if not word in result_dict and index < len(values):
                result_dict[word] = values[index]
        logging.info(f"Dictionary: {result_dict}")

        return result_dict
    

    def change_dictionary(self, dictionary, change_proportion, change_keys):
        positive_keys = [k for k, v in dictionary.items() if v > 0]
        number_of_values_to_change = int(len(positive_keys) * change_proportion)

        selected_keys = []
        if number_of_values_to_change > 0:
            selected_keys = random.sample(positive_keys, k=number_of_values_to_change)

        changes = {}
        for key in selected_keys:
            value_to_key = {v: k for k, v in dictionary.items()}
            new_value = random.randint(int(self.min_value), int(self.max_value))
            negative_key = value_to_key.get(-1 * dictionary[key])

            if change_keys:
                changes[key] = (dictionary[key], "removed")
                if negative_key is not None:
                    changes[negative_key] = (dictionary[negative_key], "removed")
                    keys = have_a_word(2, dictionary)

                    dictionary.update({keys[0]: new_value, keys[1]: -1 * new_value})
                    dictionary.pop(key, None)
                    dictionary.pop(negative_key, None)

                    changes[keys[1]] = ("non-existing", dictionary[keys[1]])
                else:
                    keys = have_a_word(1, dictionary)
                    dictionary.update({keys[0]: new_value})
                    dictionary.pop(key, None)

                changes[keys[0]] = ("non-existing", dictionary[keys[0]])

            else:
                changes[key] = (dictionary[key], new_value)
                dictionary[key] = new_value

                if negative_key is not None:
                    changes[negative_key] = (dictionary[negative_key], -1 * new_value)
                    dictionary[negative_key] = -1 * new_value

        logging.info("=== DICTIONARY UPDATED ===")
        for key in dictionary:
            if key in changes.keys():
                logging.info(f"  {key}: {changes[key][0]} → {changes[key][1]}")
        logging.info("==========================")
...
class DictSumUtil:
    def __init__(
        ...
        chunk_size: int,
        reset_cumsum: bool,
        periodic_change: bool,
        num_of_changes: int,
        change_proportion: float,
        change_keys: bool,
        random_dictionary_update: float,
        seed: Optional[int] = None,
    ):
        """
        ...
        self.dict = self.dict_creator.create_dict()
        self.horizon_length = horizon_length
        self.num_instances = num_instances
        self.chunk_size = chunk_size
        self.reset_cumsum = reset_cumsum
        self.periodic_change = periodic_change
        self.num_of_changes = num_of_changes
        self.change_proportion = change_proportion
        self.change_keys = change_keys
        self.random_dictionary_update = random_dictionary_update
        self.seed = seed

        self.extreme_values = list()
    ...
    def generate_rollout_instance(self):
        ...
        """
        # choose a random subset of keys with positive values assign from the dictionary
        positive_keys = [k for k, v in self.dict.items() if v > 0]
        if not positive_keys:
            raise ValueError("No positive-valued keys available in dictionary.")
        non_positive_keys = list(set(self.dict.keys()) - set(positive_keys))

        keys = []
        num_full_chunks = self.horizon_length // self.chunk_size
        remainder = self.horizon_length % self.chunk_size

        for _ in range(num_full_chunks):
            current_positive_keys = random.sample(positive_keys, self.chunk_size // 2)
            keys.extend(current_positive_keys)
            negative_keys = []

            for key in current_positive_keys:
                opposite_value = -1 * self.dict[key]
                for k, v in self.dict.items():
                    if v == opposite_value:
                        negative_keys.append(k)
                        break

            if len(current_positive_keys) != len(negative_keys):
                negative_keys.extend(random.choices(
                    non_positive_keys, 
                    k = abs(len(current_positive_keys) - len(negative_keys))
                ))
            keys += negative_keys

            if self.chunk_size % 2 != 0:
                key_summary = sum(value for key, value in self.dict.items() if key in keys)
                if key_summary <= 0:
                    keys.append(random.choice(positive_keys))
                else:
                    keys.append(random.choice(non_positive_keys))

        if remainder > 0:
            keys.extend(random.sample(positive_keys, remainder // 2))
            negative_keys = []
        ...
        # if horizon_length is odd, add one more random key
        if (self.horizon_length % 2) != 0:
            keys.append(random.choice(list(self.dict.keys())))
        ...
        for key in keys:
            ...
            if self.reset_cumsum and step == self.chunk_size:
                step = 0
                cumsum = 0
            step += 1

        self.extreme_values.append({min(prefix_sum), max(prefix_sum)})
        return {
            "input": keys,
            "output": prefix_sum,
            "values": values,
    ...
    def generate_rollout_instances(self):
        ...
        """
        instances, changing_steps = [], []
        if self.random_dictionary_update == 0 and self.num_of_changes and self.num_of_changes > 0 and self.change_proportion and self.change_proportion > 0:
            if self.periodic_change:
                changing_steps = np.linspace(0, self.num_instances, self.num_of_changes + 2)[1:-1]
            else:
                changing_steps = np.sort(np.random.randint(0, self.num_instances, size=self.num_of_changes))
            logging.info(f"The dictionary will be changed during the execution. The indices of changes: {changing_steps}")

        for step in range(self.num_instances):
            if len(changing_steps) > 0 and step in changing_steps:
                self.dict = self.dict_creator.change_dictionary(self.dict, self.change_proportion, self.change_keys)
            if self.random_dictionary_update > 0 and random.random() < self.random_dictionary_update:
                self.dict = self.dict_creator.change_dictionary(self.dict, self.change_proportion, self.change_keys)
