...
class DictSumExecutionExperiment(BaseExperiment):
    ...
    def _assign_config_attributes(self):
        ...
        self.reset_cumsum = self.config.reset_cumsum
        self.periodic_change = self.config.periodic_change
        self.num_of_changes = self.config.num_of_changes
        self.change_proportion = self.config.change_proportion
        self.change_keys = self.config.change_keys
        self.random_dictionary_update = self.config.random_dictionary_update
    ...
    def _initialize_cot_templates(self):
        ...
        if self.working_capacity == 1:
            self.START_TEMPLATE = (
                ...
                "Ready to start!\n"
                "IMPORTANT: Always think step by step about each calculation, showing your work. Only put the final running sum in <answer> tags, do not include any calculation inside of the <answer> tags! "
                "If more than one number is included inside of <answer> tags, the calculation is automatically incorrect!\n\n"
            )
        else:
            self.START_TEMPLATE = (
                ...
                f"Only put the final running sum in <answer> tags, do not include any calculation inside of the <answer> tags! "
                "If more than one number is included inside of <answer> tags, the calculation is automatically incorrect!\n\n"
            )
    ...
    def _initialize_non_cot_templates(self):
        ...
        if self.working_capacity == 1:
            self.START_TEMPLATE = (
                ...
                f"{self.generator_util.dict}\n"
                "Ready to start!\n"
                "IMPORTANT: DO NOT OUTPUT ANY OTHER TEXT OUTSIDE ANSWER TAGS. Do not perform any calculations. Only provide the final running sum OF ALL TURNS in <answer> tags, do not include any calculation inside of the <answer> tags! "
                "If more than one number is included inside of <answer> tags, the calculation is automatically incorrect!\n\n"
            )
        else:
            self.START_TEMPLATE = (
                ...
                f"{self.generator_util.dict}\n\n"
                f"Ready to start!\n"
                f"IMPORTANT: DO NOT OUTPUT ANY OTHER TEXT OUTSIDE ANSWER TAGS. Only provide the final running sum OF ALL TURNS in <answer> tags, do not include any calculation inside of the <answer> tags! "
                "If more than one number is included inside of <answer> tags, the calculation is automatically incorrect!\n\n"
            )
    ...
    def setup(self):
        ...
        self.generator_util = DictSumUtil(
            ...
            reset_cumsum=self.reset_cumsum,
            periodic_change=self.periodic_change,
            num_of_changes=self.num_of_changes,
            change_proportion=self.change_proportion,
            change_keys=self.change_keys,
            random_dictionary_update=self.random_dictionary_update,
            seed=self.eval_seed,
        )
        ...
        logging.info(
            f"DictSumExecutionExperiment setup complete with {len(self.ground_truth_data)} samples dynamically generated."
        )
    ...
    def prepare_prompts(self):
        ...
        prompts = self.prepare_multi_turn_prompts()
    ...
    def prepare_multi_turn_prompts(self):
        """
